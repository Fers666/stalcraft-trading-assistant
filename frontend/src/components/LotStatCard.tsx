                      ...
                      return (
                        ...
                        >
                          <TableCell sx={{ textAlign: 'left' }}>
                            ...
                            {/* Качество строки — когда в списке лоты РАЗНЫХ
                                вариантов (singleQuality === null, общая подпись
                                над таблицей не выводится). У каждого варианта
                                своя опора, поэтому без этого признака рядом
                                стоят числа с разных шкал: «Ветеран» за 659 тыс.
                                и «Легендарный» за 55 млн выглядят сравнимыми. */}
                            {!singleQuality && lot.quality_name && (
                              <Tooltip title="Качество и заточка лота: опора и цены продажи считаются по этому варианту">
                                <Box component="span" sx={{ display: 'block', color: tokens.text2, fontSize: fs.f11, cursor: 'help' }}>
                                  {lot.quality_name}
                                  {lot.enchant_level ? ` +${lot.enchant_level}` : ''}
                                </Box>
                              </Tooltip>
                            )}
