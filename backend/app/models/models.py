...
class FeedLot(Base):
    ...
    # Ключ сортировки ленты по умолчанию (P0-3, docs/tasks/ev-ranking.md).
    # profit_total отвечал на вопрос «сколько заработаю, ЕСЛИ продам» и молча
    # предполагал, что продажа состоится; замер показал, что 77.3 % строк ленты
    # продаются реже чем в 40 % случаев за 6 ч, и именно у них прибыль выше —
    # связь структурная, большую маржу забирают, продавая близко к опоре, то
    # есть глубоко в стакане. NULL = вероятность не измерена, строка уходит в
    # конец выдачи (nulls_last); подставлять p = 1 нельзя.
    ev_per_hour          = Column(Numeric(14, 2))
