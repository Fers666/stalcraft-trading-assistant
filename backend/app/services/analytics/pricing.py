...
def evaluate_lot_profit(
    ...
    даёт прибыль. Умолчание ("fast",) осталось только историческим: с ТЗ
    docs/tasks/profitability-criteria-unification.md все три вызывающих —
    лента (feed_collector), сигналы Избранного (profitable_lots) и Радар
    (market_radar) — передают TIER_ORDER. Критерий выгодности на портале один.

    Все три тира (fast, normal, premium) — ТЗ
