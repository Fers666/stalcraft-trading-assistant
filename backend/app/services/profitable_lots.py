...
def notifiable_lots(lots: list[dict]) -> list[dict]:
    ...
    (NOTIFY_TIERS) и уровень опоры (NOTIFY_REF_SCOPES). Сигнал в Redis и
    карточка остаются ПОЛНЫМИ — отфильтрованный лот виден, он лишь не будит
    телефон. Старые сигналы без ref_scope (payload в очереди на момент
