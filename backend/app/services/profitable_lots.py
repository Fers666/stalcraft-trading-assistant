...
# карточке, — но основанием разбудить телефон он не является: предметная опора
# смешивает разнородный товар и систематически завышает прибыль. Замер на проде
# 2026-08-22 («Ветка Калины», wglp): у 14 вариантов из 29 своей статистики нет,
# все получили опору 650 000, и верхний лот обещал +314.6 % — при том что у
# измеренных соседей того же предмета цены 160-564 тыс., а 650 000 набежали от
# смеси с легендарными вариантами по 57 млн. Доля таких лотов — 11 %.
NOTIFY_REF_SCOPES: tuple[str, ...] = ("variant",)
...
def notifiable_lots(lots: list[dict]) -> list[dict]:
    """
    Лоты сигнала, по которым публикуется событие уведомления.

    Два условия, и оба про доверие к числу, а не про его величину: тир
    (NOTIFY_TIERS) и уровень опоры (NOTIFY_REF_SCOPES). По тиру гейт с
    2026-08-24 не режет ничего: допуск в сам сигнал сужен до SIGNAL_TIERS, и
    лотов иных тиров в списке уже нет. Реально фильтрует опора: лот, посчитанный
    предметным фоллбеком (ref_scope="item"), остаётся виден в карточке, но не
    будит телефон. Старые сигналы без ref_scope (payload в очереди на момент
    выкатки) уведомляют по-прежнему: гейт не должен ретроактивно глушить то,
    что уже посчитано.
    """
    return [
        l for l in lots
        if l.get("tier_used") in NOTIFY_TIERS
        and l.get("ref_scope", "variant") in NOTIFY_REF_SCOPES
    ]
...
async def compute_signals_for_entry(
    ...
    Лот допускается ТОЛЬКО по цене fast (SIGNAL_TIERS), поэтому tier_used у
    всех строк один. Этим сигнал расходится с лентой и Радаром, где допуск
    трёхтировый. Ранжирование общее — по ev_profit, ожидаемым рублям, а не по
    прибыли «если продастся»: купленный лот можно выставить по любой цене,
    поэтому в ожидаемую прибыль по-прежнему идут все три сценария.
    ...
    for lot in snap.raw_lots:
        ...
        profitable.append({
            ...
            **evaluated,
            ...
            # Уровень опоры, которой посчитан лот. Дублируется в строку лота
            # (а не только в variants), потому что по нему решается, будить ли
            # пользователя, — см. notifiable_lots.
            "ref_scope":       variant["ref_scope"],
