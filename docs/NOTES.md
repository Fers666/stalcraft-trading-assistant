...
- [ ] **Внедрение Design v5 «Терминал»** — по спеке
  ...
  фазы 1–6 готовы к реализации, фаза 7 «Лента» — только после
  бэкенд-обсуждения). Эталон — прототип `design/v5/` (аудит + 9 страниц,
  завершён 2026-07-17, см. `docs/CHANGELOG.md`). Риск: фаза 1 меняет тему
  глобально — страницы News/FAQ/Admin/Register вне охвата прототипа.
