...
- [ ] **CPU-спайки: следующие шаги после прод-замеров** — по ТЗ
  `docs/tasks/cpu-spikes-recurring-2026-07-06.md` (частичный фикс
  `calculate_market_stats` реализован 2026-07-06, коммит `9a8442d`, см.
  `docs/CHANGELOG.md`). Снять прод-метрики после деплоя: длительность
  `calculate_all_market_stats` по логам worker, число уникальных
  `(item_id, region)` пар watchlist. Затем решить (с пользователем), что
  дальше: SQL-агрегация вместо ORM-загрузки всех продаж / разнесение очередей
  (collector vs analytics) / дифф-пересчёт только пар с новыми продажами.
