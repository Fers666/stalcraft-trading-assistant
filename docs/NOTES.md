...
- [x] **"Лента" переработана в "Лента возможностей" — прогноз "что добавить в Избранное сейчас" ← 2026-06-07** — раздел `/app/feed` показывает предметы всего аукциона (вне Избранного), текущая цена которых заметно ниже средней за 24ч — то есть прямо сейчас выгодный момент для закупки.
  ...
  - **Доработка по фидбеку пользователя ← 2026-06-07 (тот же день)**:
    - *Качество/заточка*: глобальный скан (`global_scanner._scan_single_item`) теперь считает `best_price`/`avg_price` ТОЛЬКО по лотам базового варианта (`additional.qlt`/`ptn` пусто или 0) — раньше в "среднюю" мешались обычные предметы и точёные артефакты +15, и сравнение "сейчас дешевле среднего" было бессмысленным. Если базовых лотов нет — скан пропускается (`return`), запись не пишется.
    ...
    - *Кнопки на карточке*: "Перейти в лоты" (навигация на `/app/lots` с `quality_filter=0, enchant_filter=0` — соответствует базовому варианту, по которому считалась цена) и "В Избранное" (`POST /watchlist/` с теми же фильтрами).
