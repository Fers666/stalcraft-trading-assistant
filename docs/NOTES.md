...
- [ ] **Роадмап подписок — оставшиеся фазы** (Phase 0 "Тарифы" реализована,
  см. `docs/CHANGELOG.md` и `docs/BUSINESS_LOGIC.md` §17): статистика в
  админке за пределами уже сделанного (rate-limit consumption, уникальные
  карточки по всем пользователям), раздел новостей, форма обращений →
  Telegram админам, "Радар рынка" (кросс-юзерная агрегация watchlist,
  отдельный аддон не по тарифам), FAQ-онбординг + копирайт лендинга под
  переименование игры в STALZONE. Черновой план — в истории чата/архиве
  планов, при возврате к теме — заново зафиксировать через `researcher`.
