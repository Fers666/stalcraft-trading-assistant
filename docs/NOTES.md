...
- [ ] **Внедрение Design v5 «Терминал»** — по спеке
  ...
  **Фазы 1–4 из 7 внедрены ← 2026-07-18** (фундамент токенов + 17 ui-компонентов,
  шелл/навбар, Избранное-эталон, Каталог+Лоты; детали — `docs/CHANGELOG.md`,
  conformance Избранного — `docs/tasks/design-v5-favorites-conformance.md`).
  **Осталось:** Фаза 5 (Радар + Склад + Настройки), Фаза 6 (Лендинг + Логин),
  Фаза 7 «Лента» — только после бэкенд-обсуждения. Эталон — прототип
  `design/v5/`. Риск: фаза 1 сменила тему глобально — страницы
  News/FAQ/Admin/Register вне охвата прототипа, проверить их конформность.
  **Известные пробелы (отложены):** медиана 7д в сайдбаре Избранного не
  выводится (нет поля в `/watchlist`); предмет `dm6l2` без иконки (нет
  исходника у EXBO — фолбэк-буква).
