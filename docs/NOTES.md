  ...
  **65 %** обещанной прибыли ленты стоит на 30 лотах с выборкой < 5 сделок; топ-10 ленты висит
  ...
  - [x] **Шаг 1 — P0-2: пол по данным и trim выбросов** — ТЗ
    ...
    **Ревизия 1 (§7 ТЗ) внедрена там же по итогам QA:** усадка к родителю удалена целиком
    ...
    ⚠ **Осталось перед выкаткой:** миграция `0039` (`market_statistics.reference_weight`)
    создана, но **на проде не применена** — `alembic upgrade head` обязателен **ДО**
    ...
    Ручки ленты не затронуты. До первого часового пересчёта колонка `NULL` → карточка
    честно показывает «low». Порядок деплоя — `docs/DEPLOY.md`;
  - [x] **Шаг 2 — P0-1: калибровка ценовых тиров по вероятности исполнения** — ТЗ
    ...
    множителей, копия в `market_stats.py` удалена (теперь импорт); фронт показывает
    «шанс продажи ~75 %» с тултипом. Формула прибыли, `evaluate_lot_profit`, пороги
    ...
    **0 %** (регресс P0-2 держится); 208 тестов зелёные, сборка фронта EXIT=0. Миграций
    нет, бюджет API не затронут. ⚠ **На прод не выкачено** (вместе с P0-2 и миграцией
    `0039` — порядок деплоя см. Шаг 1 выше).
  ...
  - [ ] Шаг 3 — P1-4: время продажи по Kaplan—Meier с цензурированием;
  - [ ] Шаг 4 — P0-3: EV-ранжирование вместо `profit_total DESC`;
  - [ ] Шаг 5 — P1-5 (стена андеркатов) и P1-6 (проверка «почему это дёшево»);
