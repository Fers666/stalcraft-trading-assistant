...
- [ ] **Emission-фича** — `GET /{region}/emission` объявлен в `StalcraftClient.get_emission()`, но никогда не вызывается. API возвращает `currentStart / previousStart / previousEnd`. Нужно: таблица `emission_events`, Celery-задача (каждые 5 мин, 1 токен), эндпоинт `/emission/current`, лента сигналов на фронте (счётчик времени с последнего выброса), Telegram-уведомления о начале выброса.
