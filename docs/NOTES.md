...
- [ ] **Внедрение Design v5 «Терминал»** — по спеке
  ...
  **Фазы 1–5 из 7 внедрены ← 2026-07-18** (фундамент токенов + 17 ui-компонентов,
  шелл/навбар, Избранное-эталон, Каталог+Лоты, Радар+Склад+Настройки+Новости;
  детали — `docs/CHANGELOG.md`, conformance Избранного —
  `docs/tasks/design-v5-favorites-conformance.md`).
  **Осталось:** Фаза 6 (Лендинг + Логин), Фаза 7 «Лента» — только после
  бэкенд-обсуждения. Эталон — прототип `design/v5/`. Риск: фаза 1 сменила тему
  ...
  **Известные пробелы (отложены, требуют backend-surface):** медиана 7д в
  сайдбаре Избранного не выводится (нет поля в `/watchlist`); P&L Склада
  (медиана /шт, P&L, текущая оценка, P&L по складу) → «—» (нет источника медиан
  по произвольным предметам склада); предмет `dm6l2` без иконки (нет исходника
  у EXBO — фолбэк-буква).
