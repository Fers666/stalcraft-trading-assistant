...
**Нормализация по базовому варианту (с 2026-06-07):** `best_price`/`avg_price` считаются
ТОЛЬКО по лотам без особого качества и заточки (`additional.qlt`/`ptn` пусто или 0) —
иначе в "среднюю" попадали бы вперемешку обычный предмет и точёный артефакт +15,
делая сравнение "сейчас дешевле среднего" бессмысленным. Если в скане нет ни одного
лота базового варианта — строка не пишется (`return` в `_scan_single_item`).
...
1. Агрегирует `global_item_scan` за 24ч: `MIN(best_price)`, `AVG(avg_price)`, группировка по `item_id`
   (нужно ≥ 2 скана в окне — `HAVING count(*) >= 2`), исключая товары из `user_feed_exclusion`
   текущего пользователя (`item_id NOT IN (...)`)
...
3. Считает `profit_pct_expr`/`profit_per_unit_expr`, отсекает товары с `liquid_lot_count <
...
**Ответ (`OpportunityItem`):** `current_price`, `avg_price_24h`, `min_price_24h`,
`est_profit_pct`, `est_profit_per_unit`, `lot_count`, `scanned_at`, `min_price_at`,
`hours_since_min` + метаданные предмета.

Цены в `global_item_scan` уже нормализованы под базовый вариант предмета — без качества
и заточки (см. выше) — поэтому сравнение корректно для артефактов и обычных предметов одинаково.
...
- **"Перейти в лоты"** — `navigate('/app/lots', { state: {..., quality_filter: 0, enchant_filter: 0} })`
  (фильтры = базовый вариант, по которому считалась цена в ленте)
- **"В Избранное"** — `POST /watchlist/` с теми же `quality_filter: 0, enchant_filter: 0`
