...
- `evaluate_lot_profit(buyout_per_unit, amount, sell_options, risk, min_margin_pct, batch_stats, tiers=("fast",))`:
  ...
  выгоден (`tier_used`). Умолчание оставлено «только `fast`», но **все три боевых вызова
  передают `TIER_ORDER`** (лента — `feed_collector`, сигналы — `profitable_lots`, Радар —
  `market_radar._count_profitable_offers`); при ≥`MIN_BATCH_SAMPLES` реальных продажах
...
- **Допуск по трём тирам (с 2026-08-22):** `evaluate_lot_profit(..., tiers=TIER_ORDER)` — лот проходит по **любому** из `fast`/`normal`/`premium` и помечается самым быстрым подошедшим (`tier_used`). Персональный порог `min_profit_margin_percent` проверяется **внутри каждого** тира, то есть не ослаблен
...
- Возвращает: `{lots, sell_options, variants, volume_7d, volatility_7d, ref, ref_source, trend, risk, total_profitable_amount, saturation_ratio, computed_at}`
  ...
  - **Событие уведомления публикуется не по всем лотам:** `notifiable_lots()` отбирает `tier_used ∈ NOTIFY_TIERS = ("fast", "normal")`, и в payload `profitable_lot` уходит `{**signal, "lots": notify}`. Ключ `signals:*` в Redis при этом пишется **полным** — `premium`-лот виден в карточке, но не будит телефон (`tasks/collectors.py`)
