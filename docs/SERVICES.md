...
|--------|-----------|--------|----------|
| `collect_all_active_lots` | каждые 20 сек, `expires=20` | `app.tasks.collectors` | Динамический batch: ceil(due/3), min=5, max=50. Проектная цель — полный цикл ≤60 сек, **фактически на проде ~110 сек** (см. «Пропускная способность» ниже) |
...
### Пропускная способность: фактический цикл ~110 с (замер прода 2026-08-07)

Таблица выше описывает проектную цель, а не факт. Реальные числа при 285 уникальных парах watchlist и включённой «Ленте артефактов»:

| Величина | Проект | Факт |
|---|---|---|
| Полный цикл обновления предмета | ≤60 с | **~99–122 с** |
| Длительность одного прогона | ≤10 с (50 × 0.2 с) | **22–38 с** |
| Расход API (оценка по логам) | — | **~340 из 400 ед/мин** (watchlist ~140 + лента 200) |

**Цикл 60 с недостижим при включённой ленте, и это ограничение внешнего API, а не сервера.** 285 пар × 2 ед за круг в 60 с = 570 ед/мин — вдвое выше лимита 400. Наблюдаемые ~110 с — равновесие, в котором обе подсистемы укладываются в лимит. Значение принято как норма (решение пользователя 2026-08-07); ускорение возможно только в обмен на бюджет ленты (`FEED_BUDGET_UNITS_PER_MIN`), а не «оптимизацией кода».

Оценка расхода снята по логам, а не по счётчику лимитера — подтвердить через `GET /admin/stats` при следующем заходе на прод.

### Срок годности периодических тиков (`expires`)

Прогон `collect_all_active_lots` (22–38 с) длиннее собственного такта (20 с), цикл ленты (58–73 с) — длиннее своего (60 с). Без `expires` неуспевшие тики копились в очереди `collector` бесконечно: замер 2026-08-06 показал **2330 задач и рост**, лаг в часы, обновление предмета раз в ~140 с и 11 подряд отменённых циклов ленты (`units=0` — задачи выполнялись не в ту минуту, на которую планировались, и попадали в окно `collect_all_history`).

С 2026-08-07 (`a6829d6`) у трёх частых задач задан срок годности: `collect_all_active_lots` — 20 с, `collect_artifact_lots` — 55 с, `collect_emission` — 110 с. Просроченный тик отбрасывается брокером: следующий соберёт те же предметы по `last_successful_check`, поэтому потери данных нет. Там же `worker_concurrency` поднят 2 → 3 (сервер четырёхъядерный; четвёртое ядро оставлено postgres и backend) — до этого потолок задавали слоты Celery, и добавление ядер на сервер эффекта не давало.

Разовая расчистка накопленного backlog: `docker compose -f docker-compose.prod.yml exec -T worker celery -A app.tasks.celery_app purge -f -Q collector`.
...
- `expected_value(scenarios)` → `int | None` (с 2026-08-22) — ожидаемая прибыль лота:
  **максимум** `int(p / 100 × value)` по сценариям `{тир: (прибыль всего лота, p_sold_6h)}`.
  Максимум, а не среднее: игрок выбирает лучшую стратегию для каждого лота отдельно.
  Сценарий без измеренной вероятности (или с `value ≤ 0`) в максимум не входит; если её нет
  ни у одного — `None`, и потребитель ставит такую строку в конец. Подставлять `p = 1`
  запрещено. **Одна на трёх потребителей** — лента (`feed_collector`), сигналы
  (`profitable_lots`), Радар (`market_radar`); копия из `feed_collector` удалена, вторая
...
- `evaluate_lot_profit(buyout_per_unit, amount, sell_options, risk, min_margin_pct, batch_stats, tiers=("fast",))`:
  перебирает переданные тиры в порядке `tiers` и возвращает **первый**, на котором лот
  выгоден (`tier_used`). Умолчание — «только `fast`». Боевые вызовы **расходятся намеренно**:
  лента (`feed_collector`) и Радар (`market_radar._count_profitable_offers`) передают
  `TIER_ORDER`, сигналы «Избранного» (`profitable_lots`) — `SIGNAL_TIERS = ("fast",)`
  (с 2026-08-24); при ≥`MIN_BATCH_SAMPLES` реальных продажах
  в том же бакете цена продажи корректируется пропорционально медианной цене пачки —
  `batch_stats` **варианта** `(qlt, ptn)` у всех потребителей (лента —
  `artifact_variant_stats`, сигналы — `compute_variant_ref` с 2026-08-22, Радар —
  не передаёт вовсе); требуемая маржа = `min_margin_pct × RISK_MARGIN_MULT[risk]`.
...
UI — «сделок дороже». Поле сохранено: как позиция цены в потоке сделок оно верно и полезно.

**Настоящая вероятность продажи — `p_sold_6h` / `p_sold_24h` (кривая дожития, фаза B,
выкачено 2026-08-17).** Каждый тир попадает в свою страту по отношению к опоре, и оттуда
читаются медиана срока среди проданных и `P(продан ≤ H)` по таблице `sale_survival`.
Прежние двенадцать чисел прогноза времени (множители `0.4 / 1.0 / 2.5` и лестница
`2/8/24 … 72/168/336` часов) не измерялись никогда и остались только фоллбеком:
`time_source = "heuristic"` в выдаче помечает именно их.

⚠ **Замеренный fill rate тиров не соответствует подписи 75 / 50 / 25.** Реальные
`P(продан ≤ 6 ч)` — **72 / 71 / 56 %** (`docs/tasks/sale-survival-curve.md` §2.4).

**Плоский участок есть, но лежит не там, где казалось.** «Быстро» и «Нормально»
различаются на **1.1 п.п.** вероятности при разнице цены 6 % — тир «Быстро» почти ничего
не покупает. «Выгодно» же стоит **15.7 п.п.** за те же +6 % цены, то есть это настоящий
размен. Дальше кривая падает круто: `ref × 1.10` → 44 %, `ref × 1.30` → 23 %.
Множители не меняются: они откалиброваны по ценам состоявшихся сделок и в этом качестве
корректны.

> Первая редакция (2026-08-17, до правки знаменателя) называла 84 / 85 / 79 % и вывод
> «верхний тир почти всегда предпочтителен». Вывод **отозван**: он был артефактом
> смещённой оценки, которая била по медленным стратам втрое сильнее и искажала форму
> кривой (§7.5 ТЗ).

**В ленте берётся другая стратификация** — по нормированной позиции плановой цены продажи
в стакане варианта. По силе признаки сопоставимы (ранговая корреляция с «продан ≤ 6 ч»
**−0.470** против **−0.444**, взаимная 0.582); лента считает по позиции не потому, что
признак сильнее, а потому, что **стакан у сборщика в руках и позиция есть даже у
вариантов без опоры** (ниже пола P0-2), тогда как в `sell_options` стакана нет.
Сырая глубина стакана (`cheaper_units`) непригодна: она немонотонна, потому что смешана
с ликвидностью варианта — «100+ единиц дешевле» даёт лучшую долю продаж просто потому,
что такие книги бывают только у оборотистых вариантов.
...
- **Допуск только по `fast` (с 2026-08-24):** `evaluate_lot_profit(..., tiers=SIGNAL_TIERS)`, `SIGNAL_TIERS = ("fast",)` — лот, выгодный лишь при выставлении по `normal`/`premium`, в сигнал **не попадает вовсе** (ни в Redis-ключ, ни в карточку, ни в уведомление), `tier_used` у всех лотов сигнала = `fast`. Ревизия 2026-08-22 переводила сигналы на `TIER_ORDER`, как в ленте, — сужено обратно как адресный поток, который приходит без спроса. **Лента и Радар не затронуты**, они по-прежнему передают `TIER_ORDER`. Персональный порог `min_profit_margin_percent` проверяется внутри тира и не ослаблен
- **Опора по варианту (с 2026-08-22):** для каждого лота берётся опора его `(qlt, ptn)` — `compute_variant_ref()`, ленивая карта `variants` (считается только для вариантов, у которых есть живые лоты). Варианты группируются из **той же** выборки `sales_30d`, которую функция уже загружает одним SELECT, тем же предикатом `_build_sales_filter`; ключ варианта с обеих сторон считает один резолвер `pricing.resolve_variant_key`. **Дополнительных SQL-запросов и запросов к Stalcraft API — ноль.** Ветка с фильтрами `quality_filter`/`enchant_filter` стала частным случаем: в карте один вариант. Если у варианта своей опоры нет (сделок не было либо опора ниже пола) — используется предметная запись `item_scope_entry` с `ref_scope: "item"`, сигнал **не теряется**
- **Статистика пачек тоже вариантная (с 2026-08-22, `321745d`):** `compute_variant_ref` кладёт в запись `batch_stats = _calculate_batch_stats(sales_variant)`, и `evaluate_lot_profit` получает `variant.get("batch_stats")`, а не `MarketStatistics.batch_stats`. Поправка на пачку — отношение `медиана_пачки / normal_price`; предметный числитель при вариантном знаменателе — отношение величин из разных выборок. Этим закрыто последнее известное расхождение с лентой (`docs/BUSINESS_LOGIC.md` §18)
- **Карта `variants` обрезается перед возвратом** (`321745d`): она считается лениво по **каждому просмотренному** лоту, а в сигнал попадают только выгодные — без обрезки в payload уезжают записи, на которые никто не ссылается (у «Ветки Калины» 29 записей на 9 лотов, **44 КБ** на ключ Redis). Остаются только `variant_key` из `lots` (прод: 497 → 59 записей, битых ссылок 0)
- ⚠ **Выборка `sales_30d` и её потребители связаны неявно.** `select()` тянет `sale_time`, `price_per_unit`, `amount`, `total_price`, `additional_info`: `amount` читает `_calculate_batch_stats`, `total_price` — `extract_time_price_pairs`. Обе функции берут поля у `Row` **по атрибуту**, поэтому недостающая колонка — это `AttributeError` на каждой watchlist-записи, а не деградация. Так и случилось 2026-08-22 (`50ea7dc`, инцидент ~12 минут). Добавляешь потребителя к этой выборке — проверь список колонок
- Профитные лоты сортируются по `ev_profit` убыв. (`None` — в конец, внутри хвоста прежний порядок по `profit_per_hour`). `ev_profit` = `pricing.expected_value(scenarios)` — общая функция ленты, сигналов и Радара
- Возвращает: `{lots, sell_options, variants, volume_7d, volatility_7d, ref, ref_source, trend, risk, total_profitable_amount, saturation_ratio, computed_at}`
  - `lots[i]` дополнительно содержит `profit`, `profit_pct`, `profit_per_hour`, `tier_used`, `sell_price_used`, `breakeven_per_unit`, `ev_profit`, `p_sold_6h` (вероятность тира, которым лот прошёл — без неё `ev_profit` в UI нечем объяснить), `variant_key` (`"{qlt}:{ptn}"`), `ref_scope` (`"variant"` | `"item"` — уровень опоры, которой посчитан лот; дублируется из `variants[k]` в саму строку, потому что по нему решается, будить ли пользователя, с 2026-08-22)
  - `variants[k]` — `{ref, ref_source, ref_confidence, ref_samples, ref_scope, trend, trend_pct, volume_7d, volatility_7d, risk, sell_options, batch_stats}` варианта. Верхнеуровневые `ref`/`sell_options`/`risk` **остались** (предметный уровень) — их читают графики и «Динамика цен»
  - **Гейт уведомления двухусловный** (`notifiable_lots()`): `tier_used ∈ NOTIFY_TIERS = ("fast",)` **И** `ref_scope ∈ NOTIFY_REF_SCOPES = ("variant",)`; в payload `profitable_lot` уходит `{**signal, "lots": notify}` (`tasks/collectors.py`). По тиру гейт после сужения допуска ничего дополнительно не режет и оставлен **страховкой** (возврат многотирового допуска не должен молча расширить поток). Реально фильтрует **опора**: лот на фоллбечной предметной опоре виден в карточке с пометкой «опора по предмету», но телефон не будит — предметная опора смешивает разнородный товар и завышает прибыль (замер: «Ветка Калины», 14 вариантов из 29 без своей статистики, все на опоре 650 000, верхний лот обещал +314.6 % при ценах измеренных соседей 160–564 тыс.). Старые payload'ы без поля уведомляют по-прежнему — `l.get("ref_scope", "variant")`. Прод после выкатки: `ref_scope` 214 `variant` / 28 `item` (11.6 %), уведомляемых 132 из 242
...
- **Асимметрия допуска и обхода — намеренная, проверена замером.** В ленту допускаются три
  тира, а горячим предмет делает **только `fast`**: порог `_load_observed_yield` —
  `FAST_RATIO`. При выкатке его подняли до `NORMAL_RATIO` (цена `normal` продаётся в 74.5 %
  случаев против 81.3 % у `fast` — выглядело настоящей выгодой) и **вернули в тот же день**:
  горячих стало ~143 из 383, при неизменном бюджете очередь перестала разгребаться за цикл,
  `deferred` вырос 6 → 70, круг растянулся до ~13 минут. После отката `deferred` 70 → 37.
- **Холодный предмет из ленты НЕ исчезает.** `delete_stale_rows` удаляет строки предмета
  только когда его обходят заново, поэтому строки холодного живут до следующего обхода
  (каждый `FEED_COLD_EVERY_N_CYCLES`-й цикл, ~15 мин — сильно раньше `FEED_STALE_ROW_HOURS`).
  Он виден в ленте всеми тремя тирами, просто его данные обновляются реже. Оружие и броня
  попадают в ленту именно так: почти все их строки — тира `premium`, а такие предметы
  холодные при любом варианте порога. Цена — часть выгодных лотов не застаём: лот живёт
  минуты.
...
7. **Сортировка и пагинация (ревизия 2026-06-29; ключ порядка изменён 2026-08-22):** после п.6 (метрики для всех бакетов до `MAX_BUCKETS=500`) — финальная сортировка по **`ev_offers_total DESC`** (`None` → `0`). `profitable_offers_count` остаётся в ответе как отображаемая метрика, но порядок страницы больше не задаёт; `watchers_count` не влияет на порядок с 2026-06-29.

8. **Ревизия 2026-08-22 — единый критерий выгодности** (ТЗ `docs/tasks/profitability-criteria-unification.md`, Этап 3; смена видимого порядка страницы подтверждена пользователем как продуктовое решение):
   - `_count_profitable_offers` зовёт `evaluate_lot_profit(..., tiers=TIER_ORDER)` — допуск по **любому** из трёх тиров, как в ленте (сигналы «Избранного» перешли на три тира тогда же, но **с 2026-08-24 сужены обратно до `fast`**; Радар этой правкой не затронут) — и возвращает **кортеж** `(profitable_offers_count, ev_offers_total)`. `ev_offers_total` = Σ `pricing.expected_value(scenarios)` по выгодным лотам бакета (сценарий тира лота берётся из `evaluated["profit"] × amount`, остальные — из цен `sell_options`; `ev = None` в сумму не идёт).
   - `_calculate_market_radar_aggregate` читает кривую дожития **один раз на пересчёт** (`load_survival(db, now)`) и передаёт её в `make_sell_options`: раньше Радар звал её без `survival`, поэтому `p_sold_6h` у опций не было вовсе. Пересчёт кэшируется 60 с, так что чтение таблицы дожития — раз в минуту.
   - **Замер на проде после фикса `9fc9953`: 228 бакетов, пересчёт агрегата 3.35 с, кэш `CACHE_TTL = 60` с.** Сравнить с прежним временем невозможно — до фикса агрегат падал целиком, поэтому критерий приёмки «прирост не более десятков миллисекунд» проверить не на чем.
   - **Фикс `9fc9953` (`AttributeError` на бакетах без фильтра):** волатильность для ветки `quality_filter IS NULL AND enchant_filter IS NULL` бралась как `stats.volatility_7d`, но у `market_statistics` колонка называется `price_volatility_7d` (`volatility_7d` есть у `artifact_variant_stats` и `feed_lots`). Ветка с фильтром считает волатильность из выборки продаж (`_volatility(prices)`) и потому не падала — дефект был ровно на бакетах `NULL/NULL`, то есть на самых массовых.

**Сигнатура и кэш:** `get_market_radar_aggregate(db: AsyncSession, page: int = 1, page_size: int = 20) -> dict`. Redis-кэш вынесен в общий `_get_cached_aggregate(db)` (ключ `market_radar:aggregate`, TTL 60 сек, не изменился) — кэшируется **весь** отсортированный список бакетов (не срез страницы), пагинация — `top_items[start:end]` в Python после чтения из кэша, без отдельного ключа на страницу и без `COUNT(*) OVER()` (метрика сортировки вычисляется в Python над JSON `raw_lots`, не SQL-колонка — пагинация на уровне SQL для неё невозможна). При недоступности Redis `_get_cached_aggregate` читает/пишет с `logger.warning` и просто пересчитывает на каждый запрос (без хард-фейла). Ответ дополнен `total_count` (длина полного списка), `page`, `page_size`. **Важно:** стоимость cache-miss (полный пересчёт `profitable_offers_count`) больше не зафиксирована на топ-20 — растёт линейно с числом бакетов до потолка `MAX_BUCKETS=500`. Замер на проде 2026-08-22: **228 бакетов, полный пересчёт 3.35 с** (прежняя оценка «~18 бакетов» устарела) — при TTL 60 с приемлемо, но запас до `MAX_BUCKETS` уже меньше половины: дальнейший рост watchlist потребует пересмотра TTL/safety-cap.

### get_watchlist_suggestions (обезличенные подсказки пустого «Избранного»)

`get_watchlist_suggestions(exclude_item_ids: set[str], limit: int = 6) -> list[dict]` (2026-07-24) — обслуживает `GET /watchlist/suggestions` (`backend/app/api/v1/endpoints/watchlist.py`). **Cache-read-only:** читает только готовый кэш через `_read_cached_aggregate()` и на промахе возвращает `[]` — тяжёлый пересчёт агрегата НЕ запускается (иначе негейтированный эндпоинт стал бы DoS-вектором для любого одобренного пользователя, security-ревью 2026-07-24; кэш греет только платный `/market-radar`). Дедуплицирует бакеты по `item_id` (max по `profitable_offers_count` и `watchers_count`), исключает `exclude_item_ids` и предметы без master-записи, ранжирует по кортежу `(has_profitable, _profit, _watchers)` убыв. (сначала выгодные сейчас, потом популярные) и обрезает до `limit`. **С 2026-08-22 `_profit` — это `ev_offers_total` бакета, а не `profitable_offers_count`** (дедуп по `item_id` берёт максимум по той же метрике): с допуском по трём тирам счётчик положителен почти у каждого бакета, и флаг перестал бы различать хоть что-то. Цена решения: бакет с выгодными лотами, но без измеренной вероятности продажи, флага не получит и уйдёт ранжироваться по популярности — тот же путь, что у случая «выгодных лотов сейчас нет». Отдаёт slim-DTO **без числовых счётчиков** — только `item_id`, `name_ru`, `name_en`, `icon_path` и два булевых флага `has_profitable` (`_profit > 0`) / `is_popular` (`_watchers >= SUGGEST_POPULAR_MIN_WATCHERS`, порог 2). Эндпоинт **НЕ гейтится** аддоном Радара (обезличенная выжимка ниже платного Радара). Формула и мотивация — `docs/BUSINESS_LOGIC.md` §«Радар рынка» (подраздел «Подсказки пустого Избранного»).

`_read_cached_aggregate() -> dict | None` — чистое чтение кэша `market_radar:aggregate` (без пересчёта); `None` = кэш пуст/недоступен. Используется `_get_cached_aggregate` и cache-read-only путём `get_watchlist_suggestions`.

`_get_cached_aggregate(db: AsyncSession) -> dict` — read-through helper Redis-кэша агрегата для платного Радара: `_read_cached_aggregate()`, при cache-miss вызывает `_calculate_market_radar_aggregate` и пишет обратно с TTL 60с. Прогрев кэша делает только этот (платный) путь — подсказки онбординга кэш не греют.
