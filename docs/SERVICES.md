...
**Группировка по варианту — каждая заточка/качество отдельно (с 2026-06-07):**
Лоты разной заточки/качества — разные товары с разной ценой (точёный артефакт +15
стоит совсем не как +0). `_scan_single_item` группирует все полученные лоты по
варианту `(additional.qlt, additional.ptn)` (нормализуется в `(0, 0)` для
базового — без качества/заточки) и пишет ОТДЕЛЬНУЮ строку скана на каждый
встреченный вариант, со своими `best_price`/`avg_price`/`liquid_lot_count`/
`tradability_score`. `quality`/`enchant` — колонки `global_item_scan` (миграция `0020`).
`prev_best_price`/`price_change_pct` сравниваются с историей ТОГО ЖЕ варианта
(`WHERE quality=... AND enchant=...`). Группировка — в памяти по уже полученным
лотам, без дополнительных запросов к API.

> Первая версия (раньше в этот же день) считала цены ТОЛЬКО по базовому варианту,
> игнорируя остальные — по фидбеку пользователя переделано: каждая заточка должна
> быть отдельной "единицей" со своей карточкой в ленте, а не игнорироваться.
...
Теперь — новая строка на каждый скан КАЖДОГО варианта (нужна для агрегации
"Топ возможностей за 24ч"). Индексы: `(item_id, region, scanned_at)` — окно 24ч,
`(item_id, region, quality, enchant, scanned_at)` — выборка истории варианта.
Чистка строк старше 120 дней — в `cleanup.delete_old_data` (как и остальные снэпшоты).

Объём: было ~17 280 строк/день (12 предметов × 1440 запусков по одной строке);
теперь умножается на число вариантов на предмет (обычно 1–5) — за 120 дней
ориентировочно несколько млн строк, до ~1 ГБ с индексами. Не критично, но при
росте каталога стоит периодически проверять размер таблицы.
...
**всего аукциона** вне Избранного, которые **прямо сейчас** выгодно купить и перепродать
позже по средней цене — с учётом комиссии аукциона при продаже.

```
est_profit_pct = (avg_price_24h × (1 − FEED_COMMISSION) − current_price) / current_price × 100
est_profit_per_unit = avg_price_24h × (1 − FEED_COMMISSION) − current_price   # FEED_COMMISSION = 0.05
```

То есть: купил сейчас по `current_price` → выставил по средней цене 24ч → после вычета 5%
комиссии аукциона получил на руки `est_profit_per_unit` с каждой единицы; в процентах от цены
покупки — `est_profit_pct`. Это и есть "выгодность", а не просто "на сколько дешевле средней":
для дешёвых предметов небольшая абсолютная скидка может давать высокий ROI, и наоборот.

*Эволюция метрики (правки по фидбеку пользователя, всё в течение 2026-06-07):*
1. `(avg − min) / avg` — просадка за сутки (могла случиться 20ч назад, уже поздно)
2. `(avg − current) / avg` — текущая скидка от средней (actionable, но без учёта комиссии)
3. **текущая**: `(avg × 0.95 − current) / current` — чистая прибыль "на руки" после комиссии,
   и именно по ней идёт ранжирование и фильтрация (`profit_per_unit_expr > 0`)

**Каждый вариант (заточка × качество) — отдельная "единица" со своей карточкой ←
правка по фидбеку 2026-06-07:** лоты разной заточки/качества — разные товары
с разной ценой, сравнивать их напрямую бессмысленно (точёный +10 не равен +0).
Вся агрегация и ранжирование идёт по составному ключу `(item_id, quality, enchant)` —
один и тот же предмет может занимать в ленте несколько мест разными заточками.

**Логика:**
1. Агрегирует `global_item_scan` за 24ч: `MIN(best_price)`, `AVG(avg_price)`, группировка по
   `(item_id, quality, enchant)` (нужно ≥ 2 скана ЭТОГО варианта в окне — `HAVING count(*) >= 2`),
   исключая товары из `user_feed_exclusion` текущего пользователя (`item_id NOT IN (...)` —
   скрытие действует на весь предмет, не на отдельный вариант)
...
4. Для топ-N отдельным запросом находит момент исторического минимума ПО ВАРИАНТУ —
   `min_price_at`/`hours_since_min` (доп. контекст: насколько вообще "качает" этот вариант);
   ключ поиска — `(item_id, quality, enchant)`, не просто `item_id`

**Ответ (`OpportunityItem`):** `quality`, `enchant`, `variant_label` (человекочитаемое
название варианта — `"Легендарный +12"` / `"+5"` / `null` для базового, считается в
`_variant_label()`: качество показывается только для артефактов), `current_price`,
`avg_price_24h`, `min_price_24h`, `est_profit_pct`, `est_profit_per_unit`, `lot_count`,
`scanned_at`, `min_price_at`, `hours_since_min` + метаданные предмета.

#### Скрытие предметов из ленты (`user_feed_exclusion`, миграция `0019`)

Пользователь может скрыть неинтересный предмет кнопкой на карточке — он пропадает из его
персональной ленты, но не из ленты других пользователей (таблица per-user, уникальность
`user_id + item_id + region`).

- `POST /monitoring/feed/exclude` `{item_id, region}` — скрыть
- `DELETE /monitoring/feed/exclude/{item_id}?region=RU` — вернуть обратно
- `GET /monitoring/feed/excluded?region=RU` — список скрытых (для интерфейса управления)

`/feed` фильтрует через `GlobalItemScan.item_id.notin_(select(UserFeedExclusion.item_id)...)`.

#### Кнопки на карточке (`FeedPage.tsx`)

- **"Перейти в лоты"** — `navigate('/app/lots', { state: {..., quality_filter: item.quality, enchant_filter: item.enchant} })`
  (фильтры = вариант КОНКРЕТНОЙ карточки, не захардкоженный базовый — чтобы попасть
  именно в ту заточку, для которой посчитана выгодность)
- **"В Избранное"** — `POST /watchlist/` с теми же `quality_filter`/`enchant_filter` варианта
- **"Скрыть"** — `POST /monitoring/feed/exclude`, убирает карточку из текущей выборки;
  диалог "Скрытые" (кнопка в шапке страницы) показывает список и позволяет вернуть обратно
