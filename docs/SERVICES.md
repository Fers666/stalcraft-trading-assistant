...
`get_watchlist_suggestions(exclude_item_ids: set[str], limit: int = 6) -> list[dict]` (2026-07-24) — обслуживает `GET /watchlist/suggestions` (`backend/app/api/v1/endpoints/watchlist.py`). **Cache-read-only:** читает только готовый кэш через `_read_cached_aggregate()` и на промахе возвращает `[]` — тяжёлый пересчёт агрегата НЕ запускается (иначе негейтированный эндпоинт стал бы DoS-вектором для любого одобренного пользователя, security-ревью 2026-07-24; кэш греет только платный `/market-radar`). Дедуплицирует бакеты по `item_id` (max по `profitable_offers_count` и `watchers_count`), исключает `exclude_item_ids` и предметы без master-записи, ранжирует по кортежу `(has_profitable, _profit, _watchers)` убыв. (сначала выгодные сейчас, потом популярные) и обрезает до `limit`. Отдаёт slim-DTO **без числовых счётчиков** — только `item_id`, `name_ru`, `name_en`, `icon_path` и два булевых флага `has_profitable` (`_profit > 0`) / `is_popular` (`_watchers >= SUGGEST_POPULAR_MIN_WATCHERS`, порог 2). Эндпоинт **НЕ гейтится** аддоном Радара (обезличенная выжимка ниже платного Радара). Формула и мотивация — `docs/BUSINESS_LOGIC.md` §«Радар рынка» (подраздел «Подсказки пустого Избранного»).
