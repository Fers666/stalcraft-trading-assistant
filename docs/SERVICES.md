...
- Возвращает: `{lots, sell_options, variants, volume_7d, volatility_7d, ref, ref_source, trend, risk, total_profitable_amount, saturation_ratio, computed_at}`
  - `lots[i]` дополнительно содержит `profit`, `profit_pct`, `profit_per_hour`, `tier_used`, `sell_price_used`, `breakeven_per_unit`, `ev_profit`, `p_sold_6h` (вероятность тира, которым лот прошёл — без неё `ev_profit` в UI нечем объяснить), `variant_key` (`"{qlt}:{ptn}"`)
  - `variants[k]` — `{ref, ref_source, ref_confidence, ref_samples, ref_scope, trend, trend_pct, volume_7d, volatility_7d, risk, sell_options}` варианта. Верхнеуровневые `ref`/`sell_options`/`risk` **остались** (предметный уровень) — их читают графики и «Динамика цен»
  - **Гейт уведомления:** `notifiable_lots()` отбирает `tier_used ∈ NOTIFY_TIERS = ("fast",)`, и в payload `profitable_lot` уходит `{**signal, "lots": notify}` (`tasks/collectors.py`). После сужения допуска (см. выше) по тиру он ничего дополнительно не режет и оставлен **страховкой**: возврат многотирового допуска не должен молча расширить поток уведомлений
