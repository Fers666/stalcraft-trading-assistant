...
- [x] **Design v5 «Терминал»: внедрение в приложение, Фазы 1–4 из 7 ← 2026-07-18** — реализация по спеке `docs/tasks/design-v5-implementation.md` (conformance-доработка Избранного — `docs/tasks/design-v5-favorites-conformance.md`). Прототип-эталон — `design/v5/`. Фазы 5–7 остаются (см. `docs/NOTES.md`).
  ...
  - **Известные пробелы (не баги, отложены по решению пользователя):** медиана 7д в строке сайдбара Избранного не выводится (нет поля в `/watchlist`); предмет «Протоартефакт» (`dm6l2`) без иконки — у EXBO нет исходника (штатный фолбэк-буква).
