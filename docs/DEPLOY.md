...
- **Деплой Arsenal Items (2026-07-31):** на рабочей машине отсутствовали `.claude/skills/prod-deploy/SKILL.md` и SSH-ключ к прод-серверу (machine-specific, не синкаются через git). Деплой пришлось выполнять вручную через разовую SSH-сессию с паролем (не сохранён). На будущее: рассмотреть либо настройку SKILL.md на всех рабочих машинах пользователя, либо закрепление деплоя за одной машиной.
