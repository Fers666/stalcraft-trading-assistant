...
### `sale_survival` — кривая дожития лота (миграция `0043`)
...
| `pct_sold_ever` | numeric(5,2) | продались когда-либо (по страте, не по горизонту) |
| `median_hours` | numeric(6,2) | медиана срока **среди проданных** |

Уникальный ключ `(feature, bucket, horizon_h)`. `pct_sold_ever` и `median_hours` от горизонта
не зависят и дублируются по строкам страты намеренно — сводка читается одним запросом.
...
| Позиция в книге | n | ≤1 ч | ≤3 ч | ≤6 ч | ≤12 ч | медиана продажи |
|---|---|---|---|---|---|---|
| верх 10 % | 4958 | 54.4 | 81.5 | **89.4** | 92.6 | 0.74 ч |
| верх 10–25 % | 1434 | 34.7 | 62.7 | 74.1 | 80.0 | 1.23 ч |
| 25–50 % | 445 | 22.9 | 55.5 | 72.8 | 80.6 | 1.71 ч |
| нижняя половина | 284 | 12.3 | 24.6 | **34.2** | 40.6 | 1.81 ч |
| книга ≤ 3 лотов | 537 | 28.9 | 50.8 | 61.1 | 70.0 | 1.26 ч |

Разброс 89.4 → 34.2 % на горизонте 6 ч — сигнал, которого в системе не было.
