...
| `additional_info` | jsonb | Доп. данные из API. Ключи: `lot_start` (ISO-строка startTime лота — для расчёта времени продажи), `qlt` (integer 0-5 — качество артефакта), `ptn` (integer 0-15 — уровень заточки; 0 = без заточки) |
