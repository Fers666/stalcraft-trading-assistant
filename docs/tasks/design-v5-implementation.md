...
**Фаза 7 — Лента (новая фича). ✅ Закрыта 2026-08-03 — другим содержанием.**
Вместо событийного потока spike/move (`/feed/events`, материализация событий) реализована
фича **«Лента артефактов»**: таблица выгодных лотов по всем ~103 артефактам каталога.
ТЗ — `docs/tasks/artifact-feed.md` (Фазы 1–6 + Ревизии 1 и 2).
Файлы: прототип `design/v5/app/feed.html` переписан с событийного на табличный,
`pages/FeedPage.tsx` / `pages/mobile/MobileFeedPage.tsx` / `components/ArtifactModal.tsx`,
бэкенд `tasks/feed_collector.py` / `services/analytics/variant_stats.py` /
`api/v1/endpoints/feed.py`, миграция `0038`.
Приёмка исходной спеки («4 типа сигналов с фильтрами») **не применима** — заменена
критериями приёмки Фаз 2/4/5 + §Р1.4 + §Р2.7 ТЗ ленты; ⚠ ни один из них ещё не подтверждён
вживую (`qa-tester` не прогонялся) — см. `docs/NOTES.md`.
Побочно: `design/v5/DIRECTION.md` §4 и `design/v5/AUDIT.md` (DEL-01) дополнены исключением
для read-only модалки-обозревателя (карточка артефакта): допустима, когда ничего не меняет,
её содержимое доступно и другим путём, закрывается по ESC/клику вне с возвратом фокуса и не
подменяет навигацию; для неё широкий вариант `.modal.wide`.
