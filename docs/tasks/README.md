...
|----|--------|------|-----------------|
| [survival-liquidity-stratum.md](survival-liquidity-stratum.md) | ⏸ | ТЗ 2026-08-29 | Добавлять ли ликвидность варианта третьим измерением страты кривой дожития (сейчас прогноз читается по `(item_class, feature, bucket, horizon_h)`, и «Магма» с «Гелием» получают один и тот же прогноз при шестикратной разнице оборачиваемости). Начинается с замера — §4; §12 содержит открытые вопросы к пользователю |
...
| [watchlist-parallel-fetch.md](watchlist-parallel-fetch.md) | ⏸ | ТЗ 2026-08-21 | Запасной вариант на случай остаточных 429 при `concurrency = 3` — разводить задачи по времени вместо порога горячих |
