...
- `ptn` — уровень заточки (1–15, если артефакт зачарован)
...
Когда карточка Избранного имеет выбранное качество и/или заточку, `/monitoring/item` и `/monitoring/sales-chart` фильтруют `SalesHistory` по `additional_info->>'qlt'` и `additional_info->>'ptn'`.
