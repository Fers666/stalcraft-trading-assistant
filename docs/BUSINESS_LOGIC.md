...
4. **Источник цены/объёма ветвится по бакету:**
   ...
   - Бакет с заданным хотя бы одним фильтром — медиана `SalesHistory.price_per_unit` за 7 дней (`sale_time >= now() - 7d`), отфильтрованная по `qlt`/`ptn` через `_build_sales_filter(quality_filter, enchant_filter)` (`backend/app/services/analytics/pricing.py`); регион не фильтруется (агрегация по всем регионам вместе). `price_window="7d"`. Из-за известного низкого покрытия qlt/ptn в `sales_history.additional_info` (2-6%, см. `docs/NOTES.md`) для большинства таких бакетов `prices` будет пустым списком → `avg_price_24h=None`, `sales_volume_24h=None` — **это ожидаемое поведение, не баг**; строка не скрывается из топа, `watchers_count` валиден независимо от наличия цены.
