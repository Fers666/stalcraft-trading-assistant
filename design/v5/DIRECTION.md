...
| `.modal-ov` + `.modal(-h/-b/-f)` | модалка (`SC_APP.openModal`) — операции с потерей невосстановимых данных; read-only карточка-обозреватель — по исключению под таблицей |
...
**Исключение для модалки — read-only карточка-обозреватель.** Модалка допустима вне правила
«потеря невосстановимых данных», только если выполнены все четыре условия: (1) она **ничего не
меняет** — показывает детализацию строки списка и не содержит ни одного действия над данными;
(2) то же содержимое **доступно и другим путём** — отдельной страницей или карточкой предмета,
модалка лишь короткий путь, а не единственный; (3) она закрывается по **ESC и клику вне**,
удерживает фокус внутри, пока открыта, и **возвращает фокус на строку-источник**; (4) она не
подменяет собой навигацию: состояние страницы под ней сохраняется, ссылка остаётся рабочей.
Для такой карточки предназначен широкий вариант `.modal.wide` (`min(1080px, 94vw)`) с
прокручиваемым оверлеем `.modal-ov.tall`; узкая `.modal` (`min(440px,100%)`) остаётся формой
подтверждения. Первый носитель исключения — карточка артефакта в «Ленте» (`app/feed.html`).
